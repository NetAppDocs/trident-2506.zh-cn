...
[role="lead"]
您可以使用 Trident 操作员手动或使用 Helm 升级 Trident。您可以从 Trident 操作员安装升级到另一个 Trident 操作员安装，或从 `tridentctl`安装升级到 Trident 操作员版本。在升级 Trident 操作员安装之前，请查看link:upgrade-trident.html#step-3-select-an-upgrade-method["选择升级方式"]。
...
.步骤
. 如果您link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["使用 Helm 安装了Trident 。"]，您可以使用 `helm upgrade trident netapp-trident/trident-operator --version 100.2506.0`一步升级。如果您没有添加 Helm 存储库或无法使用它进行升级：
+
...
NOTE: 在切换安装方法之前，请查看 link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["安装方法之间的转换"]。
