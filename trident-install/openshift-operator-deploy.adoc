...
---
= 使用 OpenShift OperatorHub 安装Trident
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果您使用 Red Hat OpenShift，则可以使用 Red Hat 认证的操作员安装NetApp Trident 。使用以下步骤从 Red Hat OpenShift 容器平台安装Trident 。
...
.步骤
. 导航至 OpenShift OperatorHub 并搜索NetApp Trident。
+
image::../media/openshift-operator-01.png[Trident操作员]
...
** 您可以添加 `nodePrep`用于配置和启用所有 OpenShift 工作节点上的 iSCSI 和 Multipath 服务的参数。
...
.步骤
. 从已安装的操作符列表中选择Trident操作符。
. 请选择是否要从运算符中删除所有操作数实例。
+

WARNING: 如果您不选中“从该运算符中删除所有操作数实例”复选框，则不会卸载Trident 。
