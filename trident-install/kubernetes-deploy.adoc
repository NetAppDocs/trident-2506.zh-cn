...
* 运行受支持的 Kubernetes 版本的受支持的 Kubernetes 集群的完整权限，并启用了功能要求。查看 link:../trident-get-started/requirements.html["要求"] 了解详情。
* 可访问受支持的NetApp存储系统。
...
选择适合您的安装方法。您还应该在做出决定之前查看 link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["方法之间的转换"] 的注意事项。



=== 使用Trident运算符

无论是手动部署还是使用 Helm，Trident 操作员都是简化安装和动态管理 Trident 资源的好方法。您甚至可以link:../trident-install/kubernetes-customize-deploy.html["自定义您的Trident操作员部署"]使用 `TridentOrchestrator`自定义资源 (CR) 中的属性。
