...
summary: 您可以手动部署Trident操作员来安装Trident。此过程适用于Trident所需的容器镜像未存储在私有镜像仓库中的安装情况。 
---
= 手动部署Trident操作员（标准模式）
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
您可以手动部署Trident操作员来安装Trident。此过程适用于Trident所需的容器镜像未存储在私有镜像仓库中的安装情况。如果您确实拥有私有镜像仓库，请使用link:kubernetes-deploy-operator-mirror.html["离线部署流程"]。
...
现在，您可以创建 `TridentOrchestrator`并安装 Trident。或者，您可以link:../trident-install/kubernetes-customize-deploy.html["自定义您的Trident安装"]使用 `TridentOrchestrator`规范中的属性。
