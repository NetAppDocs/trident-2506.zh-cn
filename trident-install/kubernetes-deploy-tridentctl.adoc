...
NOTE: 如果安装程序未成功完成或 `trident-controller-<generated id>` (`trident-csi-<generated id>`在 23.01 之前的版本中）未处于 *Running* 状态，则表示未安装平台。使用 `-d`来link:../troubleshooting.html#unsuccessful-trident-deployment-using-tridentctl["开启调试模式"]并对问题进行故障诊断。
...
[%collapsible]
====
有关强制分离的详细信息，请参阅 link:kubernetes-customize-deploy.html["自定义Trident操作员安装"]。
