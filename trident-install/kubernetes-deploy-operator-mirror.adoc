...
.开始之前
登录到 Linux 主机并验证其是否正在管理工作和link:../trident-get-started/requirements.html["支持的 Kubernetes 集群"]并且您具有必要的权限。
...
现在，您可以创建 `TridentOrchestrator`并安装 Trident。也可以进一步link:../trident-install/kubernetes-customize-deploy.html["自定义您的Trident安装"]使用 `TridentOrchestrator`规范中的属性。以下示例显示了 Trident 和 CSI 映像位于不同注册表中的安装。
