...
.步骤
. 使用 Trident Protect CLI 插件验证目标集群上是否存在 AppVault CR：
+
[source, console]
----
...
NOTE: 如果目标集群上不存在 AppVault CR，请按照 link:trident-protect-appvault-custom-resources.html["使用Trident Protect AppVault 对象来管理存储桶。"] 中的步骤创建它。

. 查看目标集群上可用的 AppVault 的备份内容，并记录 `appArchivePath`要还原的备份：
+
[source, console]
----
...
NOTE: 使用 CR 时，请确保目标集群上存在用于应用程序还原的命名空间。
...
** *metadata.name*: (_必填_) 此自定义资源的名称；请为您的环境选择一个唯一且有意义的名称。
** *spec.appVaultRef*: (_必需_) 存储备份内容的 AppVault 的名称。
** *spec.appArchivePath*: (_Required_) AppVault 内存储备份内容的路径。使用步骤 2 中的命令查看备份内容并找到 `appArchivePath`要还原的备份。
** *spec.namespaceMapping*：恢复操作的源命名空间到目标命名空间的映射。代替 `my-source-namespace`和 `my-destination-namespace`利用来自周围环境的信息。
+
例如：
...
NOTE: 如果您的应用程序使用多个命名空间，并且这些命名空间具有同名的 PVC，则快照还原操作（就地和新命名空间）将无法正常工作。所有还原的卷将具有相同的数据，而不是每个命名空间的正确数据。使用备份还原而不是快照还原，或升级到修复此问题的 26.02 版或更高版本。
