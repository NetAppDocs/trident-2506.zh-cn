...
Trident 可以通过以下两种方式之一创建 Cloud Volumes Service 卷 link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["服务类型"^]：

* *CVS-Performance*：默认 Trident 服务类型。这种性能优化的服务类型最适合重视性能的生产工作负载。CVS-Performance 服务类型是支持最小 100 GiB 大小的卷的硬件选项。您可以选择以下选项之一 link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["三个服务级别"^]：
+
** `standard`
** `premium`
** `extreme`
...
* *CVS*：CVS 服务类型提供高分区可用性，性能水平有限至中等。CVS 服务类型是一种软件选项，它使用存储池来支持小至 1 GiB 的卷。存储池最多可包含 50 个卷，其中所有卷共享池的容量和性能。您可以选择以下选项之一 link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["两种服务级别"^]：
+
** `standardsw`
...
.你需要什么
要配置和使用  https://www.netapp.com/google-cloud/netapp-volumes/["适用于 Google Cloud 的Cloud Volumes Service"^] 后端，您需要以下内容：
...
| `network` | Google Cloud 网络用于Cloud Volumes Service卷。 | “默认” 
